...
<body data-spy="scroll" data-target=".mainmenu-area" data-offset="90">
    ...
    <script>
        function validateForm() {
            var name = document.forms["validForm"]["parentname"].value;
        ...
        <?php
            $parentnameErr = $studidErr = $roomcodeErr = $emailErr = $passwordErr = $passwordconfirmErr = $err_message = "";
            $parentname  = $studid  = $roomcode = $email = $password = $passwordconfirm = "";
            $parentnameDB = $studidDB  = $roomcodeDB = $emailDB = $passwordDB = $passwordconfirmDB = 0;
    ...
    <?php
        if(($parentnameDB==1) && ($studidDB==1) && ($emailDB == 1) && ($roomcodeDB == 1) && ($passwordDB == 1) && ($passwordconfirmDB == 1)){
            $hostname = 'localhost';
            $username = "root";
            $password = "";
            ...
            // insert query
            $sql = "INSERT INTO `district_student` (`Sname`, `SID`, `TRcode`, `Semail`, `Spswrd`) VALUES ('$parentname', '$studid', '$roomcode', '$email', '$password');";
    ...
    <section class="section-padding">
                <div class="container">
                    <!-- input statements-->
                    <div align="center">
                        <h2 class="xs-font26">Parent Sign Up Form</h2><br>
                    </div>
            <form name="signupForm" onsubmit="return validateForm()" method="post" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>" enctype="multipart/form-data">  
                        ...
                        Name: <input type="text" name="parentname" value="<?php echo $parentname;?>">
                        <span class="error">* <?php echo $parentnameErr;?></span>
                        <br><br>

                        Student ID (Child): <input type="text" name="studid" value="<?php echo $studid;?>">

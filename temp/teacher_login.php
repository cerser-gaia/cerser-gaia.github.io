...
<html>
    ...
    <script>
        function validateForm() {
            var teemail = document.forms["validForm"]["teemail"].value;
            var tepswrd = document.forms["validForm"]["tepswrd"].value;
            ...
            if (teemail == "" && tepswrd == "") {
                ...
                alert("Validating form information..\n" + "Teacher Email: " + teemail + "\nTeacher Password: " + tepswrd)
                return true;
            }
        }
    </script>
        <?php
            $teemailErr = $tepswrdErr = "";
            $teemail = $tepswrd = "";
            $teemailDB = $tepswrdDB = 0;

            if ($_SERVER["REQUEST_METHOD"] == "POST") {
                if (empty($_POST["teemail"])) {
                    $teemailDB = 0;
                } else {
                    $teemail = test_input($_POST["teemail"]);
                    $teemailDB = 1;
                }

                if (empty($_POST["tepswrd"])) {
                    $tepswrdDB = 0;
                } else {
                    $tepswrd = test_input($_POST["tepswrd"]);
                    $tepswrdDB = 1;
                }
            }
        ...
        <?php
            if(($teemailDB==1) && ($tepswrdDB==1)){
                ...
                // insert query
                $sql = "SELECT `Tname` FROM `district_teacher` WHERE `Tpswrd` = $tepswrd;";
        ...
        <section class="section-padding">
                <div class="container" id="<?php echo $_GET["User"]?>">
                    <!-- input statements-->
                    <div align="center">
                        <h2 class="xs-font26">Teacher Form</h2><br>
                    ...
                    <form name="validForm" onsubmit="return validateForm()" method="post" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>" enctype="multipart/form-data">  
                        ...
                        Password: <input type="text" name="tepswrd" value="<?php echo $tepswrd;?>">
                        <span class="error">* <?php echo $tepswrdErr;?></span>
                        <br><br>

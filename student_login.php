...
<html>
    ...
    <script>
        function validateForm() {
            var stemail = document.forms["validForm"]["stemail"].value;
            var stpswrd = document.forms["validForm"]["stpswrd"].value;
            ...
            if (stemail == "" && stpswrd == "") {
                ...
                alert("Validating form information..\n" + "Student Email: " + stemail + "\nStudent Password: " + stpswrd)
                return true;
            }
        }
    </script>
        <?php
            $stemailErr = $stpswrdErr = "";
            $stemail = $stpswrd = "";
            $stemailDB = $stpswrdDB = 0;

            if ($_SERVER["REQUEST_METHOD"] == "POST") {
                if (empty($_POST["tstmail"])) {
                    $stemailDB = 0;
                } else {
                    $stemail = test_input($_POST["stemail"]);
                    $stemailDB = 1;
                }

                if (empty($_POST["stpswrd"])) {
                    $stpswrdDB = 0;
                } else {
                    $stpswrd = test_input($_POST["stpswrd"]);
                    $stpswrdDB = 1;
                }
            }
        ...
        <?php
            if(($stemailDB==1) && ($stpswrdDB==1)){
                ...
                // insert query
                $stmt = $DBH->prepare("SELECT `Sname` FROM `district_student` WHERE `Spswrd` = ?");
		        $stmt->bindParam(1, $stpswrd);
        ...
        <section class="section-padding">
                <div class="container" id="<?php echo $_GET["User"]?>">
                    <!-- input statements-->
                    <div align="center">
                        <h2 class="xs-font26">Student Form</h2><br>
                    ...
                    <form name="validForm" onsubmit="return validateForm()" method="post" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>" enctype="multipart/form-data">  
                        Email: <input type="text" name="stemail" value="<?php echo $stemail;?>">
                        <span class="error">* <?php echo $stemailErr;?></span>
                        <br><br>

                        Password: <input type="text" name="stpswrd" value="<?php echo $stpswrd;?>">
                        <span class="error">* <?php echo $stpswrdErr;?></span>
                        <br><br>
